...
function CharacterPopup({ onClose }) {
  ...
  return (
    <div className="ui-popup" style={{ right: 10, width: 400 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <div style={{ flex: 1, display: 'grid', gap: 4 }}>
              {statKeys.map((k, i) => (
                <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ width: 24, fontSize: '0.55rem', color: '#ccc', fontWeight: 600, textAlign: 'right' }}>{stats[k]}</span>
                </div>
              ))}
              <div style={{ marginTop: 6, padding: '4px 6px', background: 'rgba(0,0,0,0.3)', borderRadius: 4 }}>
                <div style={{ fontSize: '0.5rem', color: '#888' }}>HP: {hero.currentHealth}/{stats.health} | MP: {hero.currentMana}/{stats.mana}</div>
              </div>
            </div>
          </div>
        </div>
