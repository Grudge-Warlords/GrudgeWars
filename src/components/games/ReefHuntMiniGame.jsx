...
export default function ReefHuntMiniGame({ onClose, onComplete }) {
  ...
  let _cachedSkyGrad = null;
  let _cachedSkyGradCtx = null;

  function renderFarBackground(ctx, g, now) {
    const farCamX = g.camX * PARALLAX_FAR;
    const farCamY = g.camY * PARALLAX_FAR;

    if (!_cachedSkyGrad || _cachedSkyGradCtx !== ctx) {
      _cachedSkyGrad = ctx.createLinearGradient(0, 0, 0, VIEWPORT_H);
      _cachedSkyGrad.addColorStop(0, '#0a3d62');
      _cachedSkyGrad.addColorStop(0.3, '#0c5a8a');
      _cachedSkyGrad.addColorStop(0.6, '#0e7490');
      _cachedSkyGrad.addColorStop(1, '#041225');
      _cachedSkyGradCtx = ctx;
    }
