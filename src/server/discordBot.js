...
async function handleLeaderboard(interaction) {
  ...
  try {
    const [rankedResult, totalResult] = await Promise.all([
      query(
        `SELECT owner_name, wins, losses, total_battles FROM arena_teams
         WHERE total_battles > 0 ORDER BY wins DESC, losses ASC LIMIT $1`,
        [limit]
      ),
      query('SELECT COUNT(*) AS count FROM arena_teams'),
    ]);
...
async function handleStats(interaction) {
  try {
    const [accountCount, charCount, teamsResult, battlesResult] = await Promise.all([
      query('SELECT COUNT(*) as count FROM accounts'),
      query('SELECT COUNT(*) as count FROM characters'),
      query('SELECT COUNT(*) FILTER (WHERE status = $1) AS ranked, COUNT(*) FILTER (WHERE status = $2) AS unranked, COUNT(*) AS total FROM arena_teams', ['ranked', 'unranked']),
      query('SELECT COUNT(*) as count FROM arena_battles'),
    ]);

    const teams = teamsResult.rows[0];
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '🏟️ Arena Teams', value: `${teams.total}`, inline: true },
        { name: '⚔️ Arena Battles', value: `${battlesResult.rows[0].count}`, inline: true },
        { name: '🏆 Ranked Teams', value: `${teams.ranked}`, inline: true },
        { name: '📉 Unranked Teams', value: `${teams.unranked}`, inline: true },
      )
