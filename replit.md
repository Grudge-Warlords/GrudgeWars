...
- Use the Grudge attribute system (8 attributes: Strength, Vitality, Endurance, Dexterity, Agility, Intellect, Wisdom, Tactics) â€” no other stat naming conventions.
...
- Strength (STR) â€” Physical damage, defense, health, lifesteal, block chance
- Vitality (VIT) â€” Max health, health regen, damage reduction, bleed resistance
- Endurance (END) â€” Stamina, physical defense, block effectiveness, armor, CC resistance
- Dexterity (DEX) â€” Crit chance, attack speed, accuracy, evasion, crit damage
- Agility (AGI) â€” Movement speed, evasion, dodge, crit evasion
- Intellect (INT) â€” Mana, magic damage, cooldown reduction, spell accuracy
- Wisdom (WIS) â€” Magic resistance, mana, spell block, status effect reduction
- Tactics (TAC) â€” Armor penetration, block penetration, defense break, % bonus to all stats
...
- Build classification system: Legendary â†’ Warlord â†’ Epic â†’ Hero â†’ Normal (shared `CLASS_TIERS` from `src/data/classes.js`)
...
- **Screen Flow:** Standard RPG progression: Title â†’ Intro â†’ Lobby â†’ Character Creation â†’ World Map â†’ Locations â†’ Battle.
...
- **Character System:** 24 Warlord combinations (6 races Ã— 4 classes) + 2 secret heroes. 8 attributes with 0-20 level progression. Race bonuses + class starting attributes combine for base stats.
...
- **Zone Conquer/Quest System:** 29 zones Ã— 4 optional quests each.
- **Gruda Arena:** Standalone challenge mode for simplified turn-based battles.
- **Discord Integration:** Discord OAuth (guilds.join scope). Discord bot (`src/server/discordBot.js`) with 7 slash commands. Session token system. Webhook Broadcaster for gameplay events. Invite: `https://discord.gg/FtGtmxmwkh`.
...
- **Rank Badge System:** `RankBadge.jsx` with 6 tiers (Bronzeâ†’Diamondâ†’Legend). SVG star badges with tier-colored glows.
...
- **Game Compendium:** Static HTML at `public/compendium.html` â€” 8 tabbed sections covering all game data.
- **Hero Codex:** Static HTML at `public/hero-codex.html` â€” visual reference for all 26 playable heroes.
- **Grudge Online Compendium:** `GrudgeOnlinePage.jsx` with 6 tabs (Overview, Races, Classes, Attributes, Combat Math, Tips).

## Key Data Files
- `src/data/attributes.js` â€” 8 attribute definitions, derived stat formulas, build classification (imports CLASS_TIERS from classes.js)
- `src/data/classes.js` â€” 4 class definitions (Warrior, Mage, Druid, Ranger) + CLASS_TIERS
- `src/data/races.js` â€” 6 race definitions (Human, Elf, Orc, Gnome, Worge, Dwarf) with bonuses
- `src/data/enemies.js` â€” Enemy templates, locations (32 zones), zone enemy presets
- `src/data/spriteMap.js` â€” All sprite sheets, enemy sprites, VFX maps, NPC sprites, named heroes
- `src/data/equipment.js` â€” Equipment slots, tiers, weapon/armor templates, stat scaling
- `src/data/worldMapData.js` â€” World map node positions and connections
- `src/data/zoneCutscenes.js` â€” Story cutscene data for all 32 zones
- `src/data/regionWalkData.js` â€” Region walk-in animation data for 5 terrain regions
- `src/data/battleRows.js` â€” Row-based positioning system (front/back row modifiers)
- `src/data/uiSprites.jsx` â€” UI sprite sheet definitions, ICON_REGISTRY, InlineIcon component
- `src/stores/gameStore.js` â€” Central Zustand store with all game state and logic
...
- **pg** â€” PostgreSQL client (Neon DB via `GRUDGE_ACCOUNT_DB`)
- **Warlord-Crafting-Suite DB** â€” Optional second Neon PostgreSQL database (via `DATABASE_URL`) for shared crafting, inventory, resources, and profession systems
- **Google Fonts** â€” Cinzel and Jost
- **CDNFonts CDN** â€” LifeCraft font
- **Web Audio API** â€” Sound effects and music

## Viewport & Container System
- **Reference Resolution:** 1280Ã—720 (16:9) defined in `src/utils/viewport.js`
...
**Architecture:**
- `src/server/suiteDb.js` â€” Second pg Pool connecting to `DATABASE_URL` env var. Gracefully disabled if not set.
- `src/server/craftingRoutes.js` â€” Express routes under `/api/crafting/*` for account linking, recipes, materials, inventory, crafting jobs, harvesting, and professions. All authenticated via `GAME_API_GRUDA` token.
- `src/services/craftingApi.js` â€” Client-side fetch wrapper for all crafting endpoints.
- `src/components/CraftingPanel.jsx` â€” Full UI overlay with 5 tabs: Recipes (search/filter/craft), Inventory, Resources, Jobs (claim), Professions (XP bars). Includes Discord account linking flow and suite DB status indicator.
- `src/stores/gameStore.js` â€” Suite state (`suiteLinked`, `suiteGrudgeId`, `suiteGold`, `suiteResources`, `suiteInventory`, `suiteRecipes`, `suiteCraftingJobs`, `suiteProfessions`, etc.) with 12 actions. Persisted via zustand partialize.
- `src/server/dbRoutes.js` â€” Save-game syncs gold to suite DB; load-game fetches suite inventory/resources. Both are best-effort and non-blocking.

**Key endpoints:**
- `POST /api/crafting/link-account` â€” Bridge discord_id â†’ suite grudge_id
- `GET /api/crafting/recipes` â€” All recipes with optional profession/category/tier filters
- `GET /api/crafting/inventory/:grudgeId` â€” Account inventory + resources + currency
- `POST /api/crafting/craft` â€” Submit crafting job (transactional with ingredient deduction)
- `POST /api/crafting/craft/claim` â€” Claim completed crafting job
- `POST /api/crafting/harvest` â€” Harvest resources with profession XP
- `GET /api/crafting/professions/:grudgeId` â€” Profession levels per character
- `GET /api/crafting/status` â€” Suite DB connection health check
...
- **Loot Popup UX:** Staggered item reveal animations, tier-based glow box-shadows (T1â€“T6), shimmer sweep overlay, title reveal animation
...
- **Inventory UI Overhaul:** Scaleable 5Ã—4 grid container with panel background
